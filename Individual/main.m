...
vecPlot=1:5;
figure(1001)
plot( vecPlot , GammaVec_cond1(vecPlot) , 'Marker','diamond','Color',[200 25 40]/225)
hold on
plot( vecPlot , GammaVec_cond2(vecPlot) , 'Marker','square','Color',[50 30 150]/225)
...
memconf(bElemLabels,nElements,nInc,dXYa,'k-');
...
memconf(bElemLabels,nElements,nInc,dXYa,'-');
...
% Ground Acceleration
tempGA=readmatrix('LomaPrieta1989.xlsx','Sheet',1,'Range','A5:E1604');
GAinteg=tempGA'; GAinteg=GAinteg(:)';

figure(2001)
plot(.005:0.005:40,GAinteg,'k') %dt=0.0025
title('\it \bf \fontsize{18} \fontname{Times New Roman}  Ground Acceleration (LomaPrieta1989-115)')
...
figure(2003)
plot(.005:0.005:40,BendM/1000,'k') %dt=0.005
title('\it \bf \fontsize{18} \fontname{Times New Roman} Overturning Bending Moment over Node E (M_E)')
ylabel('\it \bf \fontsize{18} \fontname{Times New Roman} M_E [kN.m]') 
xlabel('\it \bf \fontsize{18} \fontname{Times New Roman} Time [s]')
set(gca,'FontSize',18)
